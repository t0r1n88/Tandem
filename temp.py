...
def processing_report():
    ...
    try:
        ...
        base_df['Наименование'] = dct_code_name.values()
        base_df['Конкурс'] = base_df['Код'] + ' ' + base_df['Наименование']
        base_df['База'] = '9 кл.'
        ...
        df_person = pd.read_excel(name_file_person, sheet_name='Абитуриенты', skiprows=8,
                                  usecols=['ФИО', 'Нуждается в общежитии', 'Формирующее подр.',
                                           'Набор ОП', 'Сдан оригинал', 'Состояние выбран. конкурса',
                                           'СНИЛС'])

        df_person = df_person[~df_person['Набор ОП'].isnull()]  # убираем тех у кого нет заявлений
        ...
        dupl_cross_df['Состояние выбран. конкурса'] = dupl_cross_df['Состояние выбран. конкурса'].apply(
            lambda x: 1 if 'Сданы ВИ' in x else 0)
        ...
        dupl_svod_df = pd.DataFrame.pivot_table(dupl_cross_df,
                                                ...
                                                values=['Сдан оригинал', 'Сироты', 'СВО', 'Целевой договор',
                                                        'Нуждается в общежитии','Состояние выбран. конкурса'],
                                                aggfunc='sum')
        ...
        single_out_df = dupl_svod_df.reindex(
            columns=['Сдано оригиналов',
                     ...
                     'Сирот чел.', 'Дети СВО', 'Целевой договор','Состояние выбран. конкурса'])
        ...
        small_df = cross_df[['Набор ОП', 'Состояние выбран. конкурса', 'for_counting']]

        # объединяем датафреймы
        union_df = base_df.merge(small_df, how='outer', left_on='Конкурс',
                                 right_on='Набор ОП')
        ...
        base_df_groupby = union_df.groupby(['Набор ОП']).agg({'for_counting': sum})
        ...
        base_df = base_df.merge(base_df_groupby, how='inner', left_on='Конкурс',
                                right_on='Набор ОП')
