...
for file in os.listdir(path):
    ...
    df = pd.read_excel(f'{path}/{file}',
                       dtype={'Серия паспорта': str, 'Номер паспорта': str, 'Номер документа о образовании': str})
    for row in df.itertuples():
        ...
        if pd.isnull(row[4]) or pd.isnull(row[5]):
            continue
        # Очищаем столбцы ФИО
        df.loc[row[0], 'Фамилия'] = row[1].strip().replace(' ', '')
        df.loc[row[0], 'Имя'] = row[2].strip().replace(' ', '')
        df.loc[row[0], 'Отчество'] = row[3].strip().replace(' ', '')
